  ...
  const handleAuth = (event: FormEvent) => {
    ...
    mutation.mutate(input as never, {
      onSuccess: (result: any) => {
        ...
        else setAuthError(result.reason === "username_exists" ? "Tên đăng nhập đã tồn tại." : "Tên đăng nhập hoặc mật khẩu không đúng.");
      },
      onError: () => setAuthError("Không thể kết nối máy chủ. Vui lòng thử lại.")
    });
  };
