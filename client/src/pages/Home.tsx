      ...
      <main className="relative z-10 min-h-screen lg:pl-[256px]">
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 xl:px-10">
          ...
          <section className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.8fr)]">
            <div className="rounded-2xl border border-white/[0.075] bg-white/[0.032] p-5 sm:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div className="flex items-center gap-2"><p className="font-display text-[15px] font-semibold tracking-[-0.02em] text-white">Today’s missions</p><span className="rounded-md bg-emerald-400/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-emerald-300">{missions.length} available</span></div><p className="mt-1.5 text-[11px] text-slate-600">Complete simple actions and collect coins.</p></div><button onClick={() => setActivityExpanded((value) => !value)} className="text-[11px] font-semibold text-violet-300 transition-colors hover:text-white">{activityExpanded ? "Show less" : "View rules"}</button></div>{activityExpanded && <div className="mt-4 rounded-xl border border-violet-300/10 bg-violet-300/[0.04] p-3 text-[11px] leading-relaxed text-slate-400"><ShieldCheck className="mr-1.5 inline h-3.5 w-3.5 text-violet-300" /> Open the partner link, complete the short step, then return here. Coins are points for this workspace and are not cash.</div>}{claimError && <p className="mt-4 rounded-xl border border-rose-300/15 bg-rose-300/[0.05] px-3 py-2 text-[10px] text-rose-200">{claimError}</p>}<div className="mt-5 space-y-2">{missions.map((mission) => {const Icon = mission.icon; const status = missionState[mission.id] ?? mission.status; const isActive = mission.id === activeMissionId; const isWaiting = status === "opened" && isActive && countdown > 0; return <div key={mission.id} className={`group flex flex-col gap-4 rounded-2xl border p-4 transition-all duration-200 sm:flex-row sm:items-center ${status === "claimed" ? "border-emerald-300/10 bg-emerald-300/[0.025]" : isActive ? "border-white/[0.1] bg-white/[0.035] hover:border-violet-300/25 hover:bg-white/[0.055]" : "border-white/[0.06] bg-white/[0.018]"}`}><div className={`mission-icon mission-${mission.tone}`}><Icon className="h-4 w-4" strokeWidth={1.8} /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-[12px] font-semibold text-white">{mission.title}</p>{status === "claimed" && <span className="rounded-md bg-emerald-400/10 px-1.5 py-0.5 text-[9px] font-bold text-emerald-300">DONE</span>}</div><p className="mt-1 text-[10px] text-slate-600">{mission.description}</p></div><div className="flex flex-wrap items-center gap-2 sm:ml-auto"><span className="whitespace-nowrap text-[11px] font-bold text-violet-200">+{mission.reward} <span className="font-medium text-slate-600">coins</span></span>{status === "claimed" ? <span className="flex h-9 items-center gap-1.5 rounded-xl bg-emerald-400/10 px-3 text-[10px] font-bold text-emerald-300"><Check className="h-3.5 w-3.5" /> Claimed</span> : status === "opened" && !isWaiting ? <button onClick={() => handleClaimMission(mission)} className="flex h-9 items-center gap-1.5 rounded-xl bg-violet-300 px-3 text-[10px] font-bold text-[#14111e] hover:bg-violet-200"><Check className="h-3 w-3" /> Claim reward</button> : <button onClick={() => handleStartMission(mission)} disabled={Boolean(activeMissionId) || isWaiting} className={`flex h-9 items-center gap-1.5 rounded-xl px-3 text-[10px] font-bold transition-all ${activeMissionId || isWaiting ? "border border-white/[0.08] bg-white/[0.03] text-slate-600" : "bg-white text-[#0c0d12] hover:bg-violet-100"}`}>{isWaiting ? <><TimerReset className="h-3.5 w-3.5 animate-pulse" /> Wait {countdown}s</> : <><ExternalLink className="h-3.5 w-3.5" /> Open link</>}</button>}</div></div>;})}</div></div>
