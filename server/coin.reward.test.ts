...
describe("Link4Sub reward flow", () => {
  it("uses the configured level URLs", () => {
    ...
    expect(REWARD_TIERS.link4m.url).toBe("https://link4m.co/st?api=6aaa2d910aef892dca0a0bf8&url=https://lumen-dash-6t6qdyoe.manus.space/");
  });

  it("exposes positive rewards and a bounded completion wait", () => {
    expect(COIN_REWARD).toBeGreaterThan(0);
