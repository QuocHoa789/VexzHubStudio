...
export const appRouter = router({
  ...
  auth: router({
    ...
    emailSignup: publicProcedure.input(z.object({ name: z.string().trim().min(2).max(80), username: z.string().trim().min(3).max(80).regex(/^[a-zA-Z0-9_]+$/), password: z.string().min(8).max(128), captchaToken: z.string().min(20), captchaAnswer: z.string().regex(/^\d+$/) })).mutation(async ({ ctx, input }) => {
      if (!verifyCaptcha(input.captchaToken, input.captchaAnswer)) return { ok: false as const, reason: "captcha_failed" as const };
      const result = await createEmailUser(input.name, input.username, input.password);
    ...
    emailLogin: publicProcedure.input(z.object({ username: z.string().trim().min(3).max(80).regex(/^[a-zA-Z0-9_]+$/), password: z.string().min(8).max(128), captchaToken: z.string().min(20), captchaAnswer: z.string().regex(/^\d+$/) })).mutation(async ({ ctx, input }) => {
      if (!verifyCaptcha(input.captchaToken, input.captchaAnswer)) return { ok: false as const, reason: "captcha_failed" as const };
      const user = await authenticateEmailUser(input.username, input.password);
